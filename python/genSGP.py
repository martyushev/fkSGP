...
def genSGP(sgpType,motionType):
    ...
    if motionType == 0:
        ...
        L,Rgt,tgt = Xx2L(X,x,np.random.randn(3),np.random.randn(3))
    elif motionType == 2:
        L,Rgt,tgt = Xx2L(X,x,np.zeros(3),np.random.randn(3))
    elif motionType == 3:
        L,Rgt,tgt = Xx2L(X,x,np.zeros(3),np.array([0, 0, 1]))
    else:
        raise ValueError('Invalid motionType')
