% generate SGP
function sgp = genSGP(sgpType,motionType)
    ...
    elseif motionType == 1
        [L,Rgt,tgt] = Xx2L(X,x,randn(3,1),randn(3,1));
    elseif motionType == 2
        [L,Rgt,tgt] = Xx2L(X,x,zeros(3,1),randn(3,1));
    elseif motionType == 3
        [L,Rgt,tgt] = Xx2L(X,x,zeros(3,1),[0;0;1]);
    else
        error('Invalid motionType');
    end
...
function [L,Rgt,tgt] = Xx2L(X,x,pgt,tgt)
